...
**Form Fields**: Always use `<mat-label>` instead of placeholders - labels are accessible and provide better UX  
...
**Validation Criteria**: No hardcoded colors/hex values, Material components for all interactive elements, proper v20 button syntax, token-based styling, keyboard navigation, cross-theme compatibility, buttons size to content (not container-width), dense but appropriately spaced layout, use mat-label instead of placeholders for form fields.
